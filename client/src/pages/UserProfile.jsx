...
const UserProfile = () => {
  ...
  // Merge dynamic user with static profile content
  const userData = {
    name: user.name,
    email: user.email,
        ...
        <div className="bg-yellow-100 rounded-xl p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm font-lato">
          <div className="flex-1">
            ...
            <p className="text-sm mb-4">
              <strong>Disabilities:</strong> {userData.disabilities.join(", ")}
            </p>
        ...
        <div className="grid md:grid-cols-3 gap-6 font-lato">
          ...
          <div className="bg-white p-6 rounded-xl shadow">
            ...
            <ul className="space-y-2 text-sm">
              {userData.currentLessons.map((lesson, i) => (
          ...
          <div className="bg-white p-6 rounded-xl shadow">
            ...
            <ul className="space-y-2 text-sm">
              {userData.recentQuizzes.map((quiz, i) => (
                <li key={i} className="flex justify-between">
          ...
          <div className="bg-white p-6 rounded-xl shadow col-span-2">
            ...
            <div className="flex flex-wrap gap-4">
              {userData.badges.map((badge, i) => (
                <div
                  key={i}
                  className="w-20 h-20 rounded-full border-2 border-yellow-500 hover:border-teal-500 transition cursor-pointer flex items-center justify-center flex-col bg-yellow-400 text-white shadow"
                >
          ...
          <div className="bg-white p-6 rounded-xl shadow h-full flex flex-col justify-between">
            <div>
              ...
              <ul className="text-sm space-y-2 text-gray-700">
                <li>
                  <span className="font-semibold">Font Size:</span> {userData.accessibility.fontSize}
                </li>
                <li>
                  <span className="font-semibold">Lesson Format:</span> {userData.accessibility.format}
                </li>
                <li>
                  <span className="font-semibold">Quiz Support:</span> {userData.accessibility.quizSupport}
                </li>
              </ul>
            </div>
          </div>
